...
// sqrt(2*PI)
#define CTEX_S2PI 2.5066282746310002

namespace ctex {

    inline float gauss(float x, int u, float sigma) {
        return (1/(sigma*CTEX_S2PI)) * exp(-1*(((x-u)*(x-u))/2*sigma*sigma));
    }
}

// cv::Mat rgb_image_to_yiq(image_rgb::Array{Float64, 3})
